...
## Preparation
 ...
 4. Download Workflow_Chef from https://github.ibm.com/bpm/Workflow_Chef to Chef Workstation. Unzip it to directory "Workflow_Chef" under "chef-repo" https://docs.chef.io/chef_repo.html directory.
...
10.0.15.4 kvm-018879.test.local kvm-018879
 ...
 Download workflow.18002.delta.repository.zip, 8.5.5-WS-WAS-FP014-part1.zip; 8.5.5-WS-WAS-FP014-part2.zip and 8.5.5-WS-WAS-FP014-part3.zip from IBM Fix Central https://www-945.ibm.com/support/fixcentral/
 ...
 Upload the IBM Business Automation Workflow installation images to the software repository. The permission for the software repository must be at least 755 for the deployment to succeed. You must include the mandatory fix pack packages and interim fixes. There are two ways to prepare software repository, one is local software repository on each Chef Client, another is remote HTTPS Server shared among Chef Clients. The root path of software repository should be configured in properties files before running shell script.<br>
For example: ibm_sw_repo=file:///opt/swRepo or ibm_sw_repo=https://9.180.111.29:9999/<br>
 a. Local software repository on Chef Client<br>
 Software repository should be set on each Chef Client. <br>
...
Database server.<br>
         For the IBM Business Automation Workflow Enterprise V18 on a single virtual machine scenario, you can install the database server before you install IBM Business Automation Workflow or as part of the product installation.
         For the IBM Business Automation Workflow Enterprise V18 on multiple virtual machines scenario, you must install the database server before you install IBM Business Automation Workflow.
...
## Running the script
Script root directory: <your directory>/chef-repo/Workflow_Chef

### Prepare properties
The properties files must be filled before running the script, the location of properties and scripts are listed below
...
### Running the script
After preparing the properties file, running the shell script.<br>
For example: ./singlenode/baw_singlenode_fresh_install.sh
