...
BAW_Single_Node_Installation_Start () {
  # sequential
    ...
    echo
    echo "$(date -Iseconds), MTASK: $LOG_SNODE_NAME TASKS List (Applyifix, Configuration, POST) starts"
...
Main_Start () {
  ...
  echo
  echo "Start to apply ifix and configure IBM Business Automation Workflow Enterprise V18 on one node."
  echo
