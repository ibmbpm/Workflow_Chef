...
#
# This script work with IBM Business Automation Workflow Cookbook project to apply interim fix packs to IBM Business Automation Workflow Enterprise on a single host.
...
BAW_Single_Node_Installation_Start () {
  ...
  echo
  echo "$(date -Iseconds), MTASK: $LOG_SNODE_NAME, there are 2 tasks to do: Applyifix, POST Action"
...
Main_Start () {
  ...
  echo
  echo "Start to apply interim fix packs tp IBM Business Automation Workflow Enterprise on one single host."
  echo
