...
# TODO: try to reuse from IM cookbook later
...
module IM
  ...
  module Helper
    ...
    def ifix_installed?(install_dir, ifix_name, user = 'root')
      Chef::Log.debug "ifix_installed? params: install_dir: #{install_dir}, ifix_name: #{ifix_name}, user: #{user}"
      cmd = "#{install_dir}/eclipse/tools/imcl listInstalledPackages | grep #{ifix_name} || true"
      cmd_out = run_shell_cmd(cmd, user)
    ...
    def ibm_installed_from_data?(imcl_cmd_dir, data_location, offering_id, offering_version = '', user = 'root')
      ...
      cmd = "#{imcl_cmd_dir}/imcl listInstalledPackages | grep #{offering_id}_#{offering_version} || true"
      cmd_out = run_shell_cmd(cmd, user)
      cmd_out.stderr.empty? && (cmd_out.stdout =~ /^#{offering_id}_#{offering_version}/)
    end
