...
# parallel
  ...
  echo
  echo "$(date -Iseconds), MTASK: $LOG_WF01_NAME Step 1 of 2 starts, TASKS List (Upgrade, Applyifix, Configuration)"

  WF02_step1 &
  tasks_baw_multinodes_install+=("$!")
  echo "$(date -Iseconds), MTASK: $LOG_WF02_NAME Step 1 of 2 starts, TASKS List (Upgrade, Applyifix)"
  echo
...
Main_Start () {
  ...
  echo
  echo "Start to Upgrade and configure IBM Business Automation Workflow Enterprise V18 on two nodes."
  echo
